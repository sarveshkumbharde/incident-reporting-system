...
exports.assignIncident = async (req, res) => {
  try {
    ...
    if (user.role !== "admin") {
      return res.status(403).json({ success: false, message: "Only admin can assign incidents" });
    }

    const authority = await User.findById(authorityId);
    if (!authority || authority.role !== "authority") {
      return res.status(400).json({ success: false, message: "Invalid authority user" });
    }

    const incident = await Incident.findByIdAndUpdate(id, { assignedTo: authorityId, status: "under review" }, { new: true });
    if (!incident) {
      return res.status(404).json({ success: false, message: "Incident not found" });
    }

    res.status(200).json({ success: true, message: "Incident assigned successfully", incident });
  } catch (error) {
